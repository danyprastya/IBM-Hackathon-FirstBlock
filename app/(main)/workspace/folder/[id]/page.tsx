...
import { ArrowLeft, Search, Plus, FileText, ChevronRight } from "lucide-react";
...
const MOCK_FOLDER_IDEAS: Record<string, { name: string; ideas: Array<{ id: string; title: string; folder: string; date: string }> }> = {
  ...
  "tech-ideas": { name: "Tech Ideas", ideas: [] },
  "side-hustles": { name: "Side Hustles", ideas: [] },
  drafts: {
    name: "Drafts",
    ideas: [{ id: "4", title: "Welcome to FirstBlock 👋", folder: "Drafts", date: "Just Now" }],
  },
};

function MobileFolderView({ folderId }: { folderId: string }) {
      ...
      <header className="px-6 pt-12 pb-2 flex items-start justify-between">
        ...
        <div className="w-10 h-10 rounded-full bg-gray-100 flex items-center justify-center overflow-hidden">
          {user?.photoURL ? (
            ...
            <span className="text-sm font-semibold text-text-secondary">{(user?.displayName || "U").charAt(0).toUpperCase()}</span>
          )}
        </div>
      </header>
          ...
          <div className="grid grid-cols-2 gap-3">
            {folderData.ideas.map((idea) => (
              <Link key={idea.id} href={`/workspace/idea/${idea.id}`} className="p-4 bg-white border border-gray-200 rounded-xl">
                ...
                <h3 className="text-sm font-semibold text-text-heading mt-1 leading-snug">{idea.title}</h3>
                <span className="inline-block mt-3 px-2.5 py-1 bg-input-bg rounded-md text-xs text-text-secondary font-medium">{idea.folder}</span>
              </Link>
            ))}
          </div>
        )}
...
function DesktopFolderView({ folderId }: { folderId: string }) {
  const folderData = MOCK_FOLDER_IDEAS[folderId] || { name: "Folder", ideas: [] };

  return (
    <div className="flex-1 overflow-y-auto p-8">
      <div className="max-w-3xl mx-auto">
        <h1 className="text-2xl font-bold text-text-heading mb-1">{folderData.name}</h1>
        <p className="text-text-secondary text-sm mb-6">{folderData.ideas.length} idea{folderData.ideas.length !== 1 ? "s" : ""}</p>

        {folderData.ideas.length === 0 ? (
          <div className="text-center py-16">
            <p className="text-text-muted text-sm mb-4">No ideas in this folder yet.</p>
            <Link href="/workspace/new" className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-accent-primary text-white text-sm font-semibold hover:bg-accent-hover transition-colors">
          ...
          <div className="space-y-2">
            {folderData.ideas.map((idea) => (
              <Link key={idea.id} href={`/workspace/idea/${idea.id}`} className="flex items-center justify-between p-4 rounded-xl border border-gray-200 hover:border-accent-primary hover:bg-input-bg transition-all">
                <div className="flex items-center gap-3">
                  <FileText className="w-5 h-5 text-text-muted" />
                  <div>
                    <h3 className="text-sm font-semibold text-text-heading">{idea.title}</h3>
                    <p className="text-xs text-text-muted">{idea.date}</p>
                  </div>
                </div>
...
export default function FolderPage() {
  ...
  return (
    <>
      <MobileFolderView folderId={folderId} />
      <div className="hidden md:flex h-screen">
        <WorkspaceLayout>
          <DesktopFolderView folderId={folderId} />
        </WorkspaceLayout>
      </div>
    </>
  );
